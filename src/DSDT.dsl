 ...
 *
 * Disassembly of origin/DSDT.aml, Sat Sep 21 22:17:06 2019
 *
...
DefinitionBlock ("", "DSDT", 2, "LENOVO", "SKL     ", 0x00000000)
{
    ...
    External (_SB_.PCI0.LPC_.WLDP, UnknownObj)    // (from opcode)
    External (_SB_.PCI0.LPCB.EC__.DCWL, UnknownObj)    // (from opcode)
    ...
    External (_SB_.SGIO, MethodObj)    // 2 Arguments (from opcode)
    ...
    Name (SS3, One)
    Name (SS4, One)
    OperationRegion (GNVS, SystemMemory, 0xAB757000, 0x0761)
    ...
    Method (OWAK, 1, NotSerialized)
    {
        ...
        If (LEqual (Arg0, 0x03))
        {
            ...
            If (LOr (And (B1B2(\_SB.PCI0.LPCB.EC.AK00,\_SB.PCI0.LPCB.EC.AK01), 0x10), And (B1B2(\_SB.PCI0.LPCB.EC.AK00,\_SB.PCI0.LPCB.EC.AK01), 0x04)))
            {
                Notify (\_SB.LID, 0x02)
            }
        ...
        If (LLess (Arg0, 0x04))
        {
            If (LOr (And (\RRBF, 0x02), And (B1B2(\_SB.PCI0.LPCB.EC.AK00,\_SB.PCI0.LPCB.EC.AK01), 0x02)))
            {
    ...
    Scope (\_GPE)
    {
        ...
        Method (_E47, 0, NotSerialized)  // _Exx: Edge-Triggered GPE
        {
            Store (0x2A, P80H)
            Sleep (0x07D0)
            If (LEqual (WDRX, 0x00))
    ...
    OperationRegion (MNVS, SystemMemory, 0xAB768018, 0x1000)
    ...
    Scope (\_SB.PCI0.LPCB.EC)
    {
        ...
        {
            Fatal (0x01, 0x80010000, 0x000114C1)
        }
        ...
        {
            P8XH (0x00, 0x95)
            If (LEqual (\_SB.PCI0.LPCB.EC.ADSL, 0x01))
            {
                Notify (\_SB.PCI0.RP01.PXSX, 0x81)
            }
        }
        ...
        {
            P8XH (0x00, 0x96)
            If (LEqual (\_SB.PCI0.LPCB.EC.ADSL, 0x00))
            {
                Notify (\_SB.PCI0.RP01.PXSX, 0x81)
            }
        }
        ...
        {
            Store (0x88, P80H)
            If (LEqual (\_SB.PCI0.LPCB.EC.DGFQ, 0x00))
            {
                Notify (\_SB.PCI0.RP01.PXSX, 0x81)
            }

            If (LEqual (\_SB.PCI0.LPCB.EC.DGFQ, 0x01))
            {
                Notify (\_SB.PCI0.RP01.PXSX, 0x81)
            }
        ...
        Scope (\_SB.PCI0.LPCB.EC)
        {
            ...
            Method (BRNS, 0, NotSerialized)
            {
                ...
                If (\_SB.PCI0.GFX0.DRDY)
                {
                    If (LEqual (VNIT, 0x1E))
                    {
                        Store (DerefOf (Index (DerefOf (Index (BRTB, 0x00)), Local0)), Local2)
                    }

                    If (LEqual (VNIT, 0x19))
                    {
                        Store (DerefOf (Index (DerefOf (Index (BRTB, 0x01)), Local0)), Local2)
                    }

                    If (LEqual (VNIT, 0x14))
                    {
                        Store (DerefOf (Index (DerefOf (Index (BRTB, 0x02)), Local0)), Local2)
                    }

                    If (LEqual (VNIT, 0x28))
                    {
                        Store (DerefOf (Index (DerefOf (Index (BRTB, 0x03)), Local0)), Local2)
                    }

                    If (LEqual (VNIT, 0x16))
                    {
                        Store (DerefOf (Index (DerefOf (Index (BRTB, 0x04)), Local0)), Local2)
                    }
            ...
            If (LAnd (\WNTF, \TATC))
            {
                Switch (ToInteger (And (Local0, 0x01FF)))
                {
                    ...
                    Case (0x01)
                    {
                        ...
                        If (LOr (LEqual (Local2, 0x00), LOr (LEqual (Local2, 0x07), LEqual (Local2, 0x04)))){}
                        ElseIf (CondRefOf (\_SB.IETM.DPTE))
                        {
                            If (And (\_SB.IETM.DPTE, 0x01)){}
                            Else
                            {
                                ShiftLeft (0x03, 0x01, Local1)
